...
export default function Cakes() {
  ...
  const handleWishlistClick = (e, item) => {
    ...
    const exists = wishlist.find((w) => w.id === item.key);
    if (exists) removeFromWishlist(item.key);
    else
      addToWishlist({
        id: item.key,
        name: item.alt,
    ...
    <>
        ...
        <div className="container">
          <div className="row align-items-center py-2 filter-row filter-row-desktop">
            ...
            <div className="col-lg-auto position-relative">
              ...
              <div className="position-relative">
                <i className="fi fi-tr-sort-alt me-5"></i>
                <select
        ...
        <div className="date-overlay">
          <div className="date-popup">
            ...
            <div className="date-footer d-flex justify-content-between mt-3">
              <button className="btn btn-danger" onClick={() => setSelectedDate(null)}>RESET</button>
      ...
      <div className="container py-4">
        <div className="row g-5">
          {images.map((item) => (
            <div className="col-12 col-sm-12 col-md-6 col-lg-3" key={item.key}>
              <div className="cake-card" style={{ cursor: "pointer" }} onClick={() => navigate(`/cake/${item.key}`)}>
                <span className="badge-seller">Best Seller</span>

                <div className="wishlist-icon" onClick={(e) => handleWishlistClick(e, item)}>
                  <i className={`bi ${wishlist.find((w) => w.id === item.key) ? "bi-heart-fill text-danger" : "bi-heart"}`}></i>
                  <span className="wishlist-text">
                    {wishlist.find((w) => w.id === item.key) ? "Remove from Wishlist" : "Add to Wishlist"}
                  </span>
                </div>
